...
*Note:* This supersedes the ``SettingsDB`` object of 3.0. Within Sopel
...
class SopelDB(object):
    """
    Return a SopelDB object configured with the options in the given Config
    ...
    def __nonzero__(self):
        """Allow for testing if a db is set up through `if sopel.db`."""
    ...
    def check_table(self, name, columns, key):
        """
        Return ``True`` if the SopelDB contains a table with the same ``name``
    ...
    def add_table(self, name, columns, key):
        ...
        SopelDB object. If a table with the same name and key already exists,
        ...
        (with the exception of other tables) of the ``SopelDB`` object, nor
        ...
        custom query, and then creating the SopelDB object.
        """
...
class Table(object):
    """
    Return an object which represents a table in the given SopelDB, with the
